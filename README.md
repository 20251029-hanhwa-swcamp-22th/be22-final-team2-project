...
```
  [브라우저]
     │ https://app.salesboost-team2.site
     ▼
┌─────────────┐
│ CloudFront  │ ← S3 (정적 Vue 번들) + ALB (/api/*, /.well-known/*)
│ (app Dist)  │    OAC, SPA fallback 403/404 → /index.html, no-cache on index.html
└──────┬──────┘
       │ (HTTPS, origin domain = api.salesboost-team2.site for SNI match)
       ▼
┌─────────────┐
│    ALB      │ ← ACM TLS, target-type: ip, access logs → S3 team2-alb-logs
│ (EKS ing)   │    hosts: api + app.salesboost-team2.site (둘 다 gateway 로 라우팅)
└──────┬──────┘
       │
       ▼
┌─────────────┐
...
└──────┬──────┘
  ...
  ▼    ▼            ▼             ▼             ▼
┌─────┐ ┌──────┐ ┌────────┐ ┌──────────┐  ┌───────────────────────┐
│Auth │ │Master│ │Activity│ │Documents │  │ Linux Docker MariaDB  │
│:8011│ │:8012 │ │ :8013  │ │  :8014   │  │ (NAT EIP → :8001 → DB)│
└─────┘ └──────┘ └────────┘ └──────────┘  └───────────────────────┘
  │ IRSA              ↕ Feign (Bearer JWT)         ▲
  │ S3 team2-         ↕ Feign (X-Internal-Token)   │ NAT EIP 2 개만 ACCEPT
  │ files-prod        ↕ JWKS                        │ TLS verify-full
  └──── JWKS fetch ───┴─────────────────────────────┘
```

**외부 노출**: CloudFront (443) + ALB (443) 만. 백엔드 Pod 는 EKS 내부, Linux MariaDB 는 iptime 라우터 뒤 Docker 컨테이너 — NAT EIP 2 개만 allowlist.
...
| 기술 | 용도 |
|---|---|
| AWS EKS | 현재 운영 백엔드 클러스터 (ap-northeast-2, `team2-prod`) |
| AWS Load Balancer Controller | ALB Ingress 프로비저닝 (IRSA) |
| S3 + CloudFront + Route 53 + ACM | 정적 프론트 (SPA) + CDN (첨부파일) + DNS + TLS |
| AWS SDK for Java v2 (S3) | auth/master 파일 업로드 (IRSA 인증) |
| ArgoCD | GitOps 자동 배포 (EKS `argocd` ns) |
| ArgoCD Image Updater | ghcr.io 이미지 태그 자동 감지 → overlays/eks push |
| GitHub Actions | 백엔드 CI (이미지 빌드 → ghcr.io); 프론트 deploy-s3.yml (OIDC Role 로 S3 sync + CF invalidation) |
| Kustomize | k8s 매니페스트 관리 (base/overlays/eks) |
| Docker | 컨테이너 빌드 (multi-stage, non-root) |
| Linux Docker MariaDB 11 | DB 서버 (playdata4, TLS verify-full, NAT EIP allowlist, 외부 포트 8001→3306) |
| k3s (legacy) | 2026-04-28 까지 D+7 롤백 창 보존, 그 후 철거 |
...
### Phase 1/2 — Docker Compose (완료, legacy)
> 로컬 개발 + 초기 스테이징.

### Phase 3 — k3s + nginx-ingress + ArgoCD (2026-04-20 컷오버로 종료, D+7 보존 후 철거)
> Linux 박스(`playdata4.iptime.org`) k3s 단일 노드. 현재는 **Deployment replicas=0 + StatefulSet/PVC 보존** 상태로 롤백 창만 유지. D+7(2026-04-28) 만료 후 정리 → [`docs/d+7-cleanup-checklist.md`](docs/d+7-cleanup-checklist.md).

### Phase 4 — AWS (2026-04-20 ~ 현재 운영)

**도메인**: `salesboost-team2.site` (Route 53 Hosted Zone). **리전**: Seoul(ap-northeast-2) + Virginia(us-east-1, CloudFront 인증서용).
...
```
         ...
         ┌─────────┼──────────────┐
         │         │              │
  app.salesboost   api.salesboost  cdn.salesboost
         │         │              │
         ▼         ▼              ▼
   CloudFront     ALB            CloudFront
 (E1NUT5BB6DLB2G) (ap-northeast-2) (E3KYUTS0POZBJ6)
         │         │              │
   ┌─────┴─────┐   │              └──► S3 team2-files-prod (OAC)
   ▼           ▼   │
 S3 team2-    ALB (via api.salesboost — SNI match)
 frontend-prod    │
 (OAC, SPA)       ▼
            EKS Ingress (ALB Controller)
                ...
                ┌────────┼────────┬────────┬────────┐
                ▼        ▼        ▼        ▼        ▼
              Auth    Master   Activity  Docs   Gateway
             (IRSA)   (IRSA)   (8013)   (8014)  (8010)
               │        │
               └────────┴───► S3 team2-files-prod (RW, SSE-AES256)
                         │
                NAT GW (EIP 3.37.149.129 primary + 43.200.233.239 fixed secondary)
                         │
                         ▼
          playdata4.iptime.org:8001 ──► Linux host:3306 ──► Docker MariaDB 11.8
                                             TLS verify-full (ACM SAN DNS:playdata4.iptime.org)
                                             iptables DOCKER-USER: 두 NAT EIP 만 ACCEPT
```
...
| 컴포넌트 | 리소스 / ID |
|---|---|
| EKS | 클러스터 `team2-prod`, VPC `vpc-0c059d1cb72e1309c`, node group `ng-default` (t3.medium, min=1 / max=3) |
| NAT Gateway | `nat-0409aa0cd28c7fb92` · primary EIP `3.37.149.129` + fixed secondary `43.200.233.239` |
| ALB | `k8s-team2-team2ing-4dc7d0a7c3-363748482.ap-northeast-2.elb.amazonaws.com` (HZ `ZWKZPGTI48KDX`) |
| CloudFront (app) | `E1NUT5BB6DLB2G` / `dl0a18n8alv5z.cloudfront.net`. Origin 1: S3 team2-frontend-prod (OAC). Origin 2: `api.salesboost-team2.site` (ALB, SNI match). Behavior: `/api/*` + `/.well-known/*` → ALB, `/index.html` + `/` → S3 |
| CloudFront (cdn) | `E3KYUTS0POZBJ6` / `d17e6a3476i5wm.cloudfront.net`. S3 team2-files-prod (OAC) |
| ACM Seoul (ALB) | `arn:aws:acm:ap-northeast-2:392484366116:certificate/38582e7d-042c-4751-bab9-4765d54717c7` (api.salesboost-team2.site) |
| ACM Virginia (CF) | `arn:aws:acm:us-east-1:392484366116:certificate/bb8715c1-98c5-45f9-b807-f3c127d94950` (app + cdn.salesboost-team2.site) |
| S3 버킷 4종 | team2-frontend-prod · team2-files-prod · team2-alb-logs · team2-db-backup-prod (모두 BPA + SSE-AES256, 로그 제외 Versioning) |
| VPC Endpoint | `vpce-0cd1cc0ddb35831f7` (S3 Gateway, 3 private RTBs — NAT 경유 없는 S3 트래픽) |
| ArgoCD (EKS) | argocd ns, image-updater 포함. Application `salesboost-eks` watch `k8s/overlays/eks` |

#### IRSA / OIDC Role

| Role | 바인딩 | 권한 |
|---|---|---|
| `team2-prod-alb-controller` | `kube-system/aws-load-balancer-controller` | ALB Controller v3.x IAM policy |
| `team2-prod-backend-auth-s3` | `team2/backend-auth` | s3:Put/Get/Delete/List team2-files-prod |
| `team2-prod-backend-master-s3` | `team2/backend-master` | 동상 |
| `GHA-Frontend-Deploy` | GitHub OIDC, `team2-frontend` main branch | s3:Sync team2-frontend-prod + CloudFront CreateInvalidation (app Dist 만) |
| `team2-db-backup` (IAM user) | Linux cron 3AM | s3:PutObject team2-db-backup-prod (write-only) |

#### 운영 URL

| 용도 | URL |
|---|---|
| 사용자 접속 | `https://app.salesboost-team2.site/` |
| 직접 API 호출 | `https://api.salesboost-team2.site/api/...` (CF 미경유, ALB 직결) |
| 첨부 파일 / 이미지 | `https://cdn.salesboost-team2.site/...` |
| ArgoCD (EKS) | k3s Phase 3 의 `:8043` 는 legacy. EKS 쪽은 `kubectl port-forward` 나 내부 도메인으로 접근 |

#### DB 접근 경로 (EKS → Linux Docker MariaDB)

- JDBC URL: `jdbc:mariadb://playdata4.iptime.org:8001/team2_<schema>?sslMode=verify-full&serverSslCert=/etc/mariadb-tls/ca.pem`
- 외부 포트 **8001** → Linux host 3306 → Docker MariaDB 3306 (iptime 공유기 포트포워딩 기존 룰 재활용 — MariaDB docs 상 8006 스펙은 미사용)
- TLS: ACM SAN `DNS:playdata4.iptime.org` 매칭, verify-full 호스트네임 검증 통과
- 방화벽: Linux iptables DOCKER-USER 가 conntrack `ctorigdstport 8001` 매칭으로 EIP 2개 (`3.37.149.129`, `43.200.233.239`) 만 ACCEPT, 그 외 DROP

#### 자동화된 백업

- **스크립트**: `/srv/mariadb/backup.sh` (Linux) — `mariadb-dump --single-transaction --databases team2_auth team2_master team2_activity team2_docs`, gzip, S3 업로드, 7일 로테이션
- **크론**: `0 3 * * *` (매일 KST 03:00) → S3 `team2-db-backup-prod/<YYYY-MM-DD>/`
- **롤백 덤프**: `/srv/mariadb/backup/pre-migrate-2026-04-20.sql.gz` (프리 컷오버 스냅샷, D+7 까지 보존)
- **IAM**: `team2-db-backup` 사용자 — PutObject + ListBucket **write-only**. 복구 시 별도 read principal 필요

> 상세 레퍼런스: [`docs/aws-migration-guide.md`](docs/aws-migration-guide.md) (실행 가이드), [`docs/d+7-cleanup-checklist.md`](docs/d+7-cleanup-checklist.md) (D+7 정리 절차)
> 구 k3s 계획 문서: [`docs/k8s-전환-및-CICD-전략.md`](docs/k8s-전환-및-CICD-전략.md), [`docs/deployment-plan.md`](docs/deployment-plan.md)
...
```bash
# EKS ArgoCD (kubectl port-forward -n argocd svc/argocd-server 8080:443 → localhost:8080)
# → salesboost-eks → History → 이전 revision → Rollback

# 또는 CLI
argocd app rollback salesboost-eks <revision>
kubectl --context eks-team2-prod -n team2 set image deploy/서비스명 서비스명=ghcr.io/...:이전SHA

# 완전 롤백 (EKS → k3s, D+7 내에만 가능):
# 1) Route53 ALIAS 를 k3s 쪽으로 되돌리거나 k3s FE 복구
# 2) /srv/mariadb/backup/pre-migrate-2026-04-20.sql.gz 로 Linux DB 복구 (필요 시)
# 3) k3s Deployment replicas 복구: kubectl -n team2 scale deploy/... --replicas=1
# 4) k3s StatefulSet team2-mariadb 복구 및 PVC 재연결
# 상세: docs/d+7-cleanup-checklist.md
```
