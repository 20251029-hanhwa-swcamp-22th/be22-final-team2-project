...
<table align="center">
  ...
  <tr>
    <td align="center">
      <sub>MSA 설계 / 인증·인가<br/>Activity · Auth 서비스<br/>CI/CD · 인프라</sub>
    </td>
    <td align="center">
      <sub>Activity 서비스<br/>Frontend (Vue 3)<br/>서비스 간 통신 연동</sub>
    </td>
    <td align="center">
      <sub>Documents 서비스<br/>Master 서비스<br/>PDF 생성 · 결재 워크플로우</sub>
    </td>
  </tr>
</table>
...
### Phase 1/2 — Docker Compose (현재)

| 서비스 | 포트 | 비고 |
|---|---|---|
| Frontend (nginx) | **8001** (외부) | 유일한 외부 노출 포트 |
| Gateway | 8010 (내부) | expose only |
| Auth | 8011 (내부) | healthcheck (curl JWKS) |
| Master | 8012 (내부) | |
| Activity | 8013 (내부) | |
| Documents | 8014 (내부) | |
| MariaDB | 8006 (외부) / 3306 (내부) | 운영 시 외부 포트 제거 검토 |

### Phase 3 — Kubernetes (계획)

```
Route53 → ALB → Ingress (nginx-ingress)
                  ├─ /api/* → Gateway Service
                  └─ /*     → Frontend Service
