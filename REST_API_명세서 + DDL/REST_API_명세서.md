...
| `UserStatus` | `active`, `on_leave`, `retired` | 사용자 재직 상태 |
| `ClientStatus` | `active`, `inactive` | 거래처 상태 |
| `ItemStatus` | `active`, `inactive` | 품목 상태 |
| `ActivityType` | `meeting`, `issue`, `memo`, `schedule` | 활동 유형 |
| `ActivityPriority` | `high`, `normal` | 활동 우선순위 |
| `PI/PO Status` | `draft`, `confirmed`, `pending_approval`, `rejected`, `cancelled`, `registration_requested`, `modification_requested`, `deletion_requested` | PI/PO 문서 상태 |
| `ProductionOrderStatus` | `in_progress`, `completed` | 생산지시서 상태 |
| `ShipmentOrderStatus` | `preparing`, `completed` | 출하지시서 상태 |
| `ShipmentStatus` | `preparing`, `completed` | 출하현황 상태 |
| `CollectionStatus` | `unpaid`, `paid` | 수금 상태 |
| `CI/PL Status` | `pending`, `completed` | CI/PL 상태 |
| `ApprovalStatus` | `pending`, `approved`, `rejected` | 결재 상태 |
| `ApprovalRequestType` | `registration`, `modification`, `deletion` | 결재 요청 유형 |
| `ApprovalDocumentType` | `PI`, `PO` | 결재 문서 유형 |
| `EmailStatus` | `sent`, `failed` | 이메일 상태 |
...
| `user.userId` | `Integer` | 사용자 ID |
| `user.userName` | `String` | 사용자 이름 |
| `user.userRole` | `String` | 역할 (admin/sales/production/shipping) |
| `user.departmentName` | `String` | 소속 부서명 |
| `user.positionName` | `String` | 직급명 |
...
| 상태 코드 | 조건 |
|-----------|------|
| `400 Bad Request` | 이메일/비밀번호 불일치 |
| `400 Bad Request` | 재직(`active`) 상태가 아닌 사용자 |

**비고**: `canLogin()` 메서드에 의해 `UserStatus.ACTIVE` 상태인 사용자만 로그인 가능하다. 토큰 갱신(refresh) 응답에도 동일한 user 객체가 포함된다.
...
#### `GET /api/users` — 사용자 목록 조회 (페이징) ✅ 구현완료

사용자 목록을 페이징하여 조회한다. 검색 조건을 조합할 수 있다.
...
| `size` | `int` | - | `10` | 페이지 크기 |
| `userName` | `String` | - | - | 사용자명 검색 (부분 일치) |
| `departmentId` | `Integer` | - | - | 부서 ID 필터 |
| `userRole` | `String` | - | - | 역할 필터 (`admin` / `sales` / `production` / `shipping`) |
| `userStatus` | `String` | - | - | 상태 필터 (`ACTIVE` / `ON_LEAVE` / `RETIRED`) |

**응답 `200 OK`**: `PagedResponse<UserListResponse>`
...
#### `PUT /api/users/{id}/password` — 비밀번호 변경 ✅ 구현완료

사용자가 자신의 비밀번호를 변경한다. 현재 비밀번호 확인 후 새 비밀번호로 변경한다.
...
| 상태 코드 | 조건 |
|-----------|------|
| `400 Bad Request` | 현재 비밀번호가 일치하지 않음 |
| `404 Not Found` | 해당 사용자가 존재하지 않음 |

---

#### `POST /api/users/{id}/password/reset` — 관리자 비밀번호 초기화 ✅ 구현완료

관리자가 사용자의 비밀번호를 초기값(`test1234`)으로 초기화한다.
...
**비고**: 초기화 비밀번호는 `test1234`이며, 사용자에게 변경을 안내해야 한다.

---

#### `POST /api/auth/forgot-password` — 비밀번호 찾기 ✅ 구현완료

등록된 이메일로 임시 비밀번호를 생성하여 발송한다.
...
**비고**: 랜덤 임시 비밀번호를 생성하여 해당 이메일로 발송한다. 인증 헤더 불필요.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `status` | `String` | O | `ACTIVE` / `ON_LEAVE` / `RETIRED` |
...
| 상태 코드 | 조건 |
|-----------|------|
| `400 Bad Request` | `RETIRED` 상태 사용자의 상태 변경 시도 |
| `404 Not Found` | 해당 사용자가 존재하지 않음 |

**비고**: `RETIRED` 상태인 사용자는 상태를 변경할 수 없다. (`IllegalStateException: "퇴직한 사용자의 상태는 변경할 수 없습니다."`)
...
#### `PUT /api/positions/{id}` — 직급 수정 ✅ 구현완료
...
**응답 `200 OK`**: 수정된 `Position` 객체

**에러 응답**:

| 상태 코드 | 조건 |
|-----------|------|
| `404 Not Found` | 해당 직급이 존재하지 않음 |

---

#### `DELETE /api/positions/{id}` — 직급 삭제 ✅ 구현완료
...
#### `PUT /api/departments/{id}` — 부서 수정 ✅ 구현완료
...
#### `POST /api/company/seal` — 회사 인감 이미지 업로드 ✅ 구현완료
...
| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `file` | `File` | O | 인감 이미지 파일 (PNG/JPG) |

**응답 `200 OK`**:

```json
{
  "companySealImageUrl": "/images/seal_20260402.png",
  "message": "인감 이미지가 업로드되었습니다."
}
```
